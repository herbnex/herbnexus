...
const SubscriptionForm = ({ clientSecret }) => {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			if (error) {
				...
				setRedirecting(true);
				setTimeout(() => {
					window.location.replace("https://pawan-updates--herbnexus.netlify.app/contact"); // Update with your actual URL
	...
	return (
		<Form onSubmit={handleSubmit} className="subscription-form">
			...
			<Button type="submit" disabled={!stripe || loading || redirecting} className="subscribe-button mt-3">
				{loading || redirecting ? (
					<>
...
const Subscription = () => {
	...
	useEffect(() => {
		const fetchClientSecret = async () => {
			if (user) {
				try {
					const response = await axios.post("/.netlify/functions/create-payment-intent", { userId: user.uid });
		...
		<Container className="subscription-container">
			<Row>
				<Col md={6} className="subscription-faq">
					...
					<p>
						Your subscription includes 24/7 access to accredited herbal practitioners, personalized
