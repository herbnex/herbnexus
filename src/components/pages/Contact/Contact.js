...
const Contact = () => {
  ...
  const joinRoomById = async (roomId) => {
    ...
    const filteredParticipants = (isDoctor ? activeUsers : onlineDoctors).filter(
      (participant) =>
    ...
    const downloadFile = () => {
      const filePath = decodeURIComponent(
        new URL(downloadModal.fileUrl).pathname
          .split("/o/")[1]
          .split("?alt=media")[0]
      );
      fetch(
        `https://us-central1-health-de695.cloudfunctions.net/getDownloadUrl?filePath=${encodeURIComponent(
          filePath
        )}`
      )
    ...
    return (
      <Container fluid className="chat-room">
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center">
                      <img
                        ...
                        } // Replace "default-avatar-url" with the actual default avatar URL
                        alt="Avatar"
            ...
            {selectedParticipant ? (
              <>
                <div className="chat-top-bar d-flex justify-content-between align-items-center p-2 bg-light">
                  <div className="d-flex align-items-center">
                    <div
                      className="user-avatar bg-secondary rounded-circle me-2"
                      style={{ width: "40px", height: "40px" }}
                    >
                      <img
                        src={
                  ...
                  <div className="chat-icons">
                    ...
                    <Dropdown>
                      ...
                      <Dropdown.Menu>
                        <Dropdown.Item
                          onClick={() =>
                            history.push(`/doctor/${selectedParticipant.id}`)
                          }
                        >
                          View Profile
                        </Dropdown.Item>
                        ...
                        {showCallModal && (
                          <>
                            <div className="video-label">
                              You ({user.displayName || "Anonymous"})
                            </div>
                            <video
                              ref={localVideoRef}
                              autoPlay
        ...
        <Modal
          ...
          onHide={() => setDownloadModal({ show: false, fileUrl: "", fileName: "" })}
        >
          <Modal.Header closeButton>
          ...
          <Modal.Body>
            ...
            <Button
              variant="danger"
              onClick={() => setDownloadModal({ show: false, fileUrl: "", fileName: "" })}
            >
              No
            </Button>
          </Modal.Body>
