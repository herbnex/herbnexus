...
import { get, set } from 'https://edge.netlify.com/kv-storage';
...
export default async (request, context) => {
  ...
  try {
    let rateLimitData = await get(rateLimitKey);
    if (!rateLimitData) {
      rateLimitData = { count: 1, lastRequest: currentTime, blockedUntil: 0 };
      await set(rateLimitKey, rateLimitData);
    ...
    if (currentTime - rateLimitData.lastRequest < 60) {
      if (rateLimitData.count >= 5) {
        ...
        await set(rateLimitKey, JSON.stringify(rateLimitData));
